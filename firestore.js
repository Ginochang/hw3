document.addEventListener('DOMContentLoaded', async function(event) {
  ...
  document.querySelector('.addInterest').addEventListener('submit', async function(event) {
    ...
    if (todoText.length > 0) {
      ...
      document.querySelector('.todos').insertAdjacentHTML('beforeend', `
        <div class="todo-${todoId} py-4 text-xl border-b-2 border-purple-500 w-full">
          <a href="#" class="done p-2 text-sm bg-green-500 opacity-20 text-white">✓</a>
          ${todoText}
        </div>
      `)
      ...
      document.querySelector(`.todo-${todoId} .done`).addEventListener('click', async function(event) {
        ...
        document.querySelector(`.todo-${todoId}`).classList.remove('opacity-20')
